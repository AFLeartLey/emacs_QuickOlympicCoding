...
| Kill running process | ✅ Done | `C-c M-q k`, platform-aware (Windows/Linux) |
...
1. **Add a test**: `C-c M-q n` → an edit buffer opens; paste the sample input and
   press `C-c C-c`. Repeat to add more tests. Tests are stored in `A.cpp.tests`.
2. **Compile & run**: `C-c M-q r` → the test panel appears on the right and all
...
### Source buffer (`quickolympic-mode`, prefix `C-c M-q`)

| Key | Command | Action |
|---|---|---|
| `C-c M-q r` | `quickolympic-run` | compile & run all tests |
| `C-c M-q R` | `quickolympic-run-current-test` | run only the current test |
| `C-c M-q p` | `quickolympic-toggle-panel` | show / hide the test panel |
| `C-c M-q n` | `quickolympic-new-test` | add a new test (then edit its input) |
| `C-c M-q e` | `quickolympic-edit-test` | edit the current test |
| `C-c M-q k` | `quickolympic-kill-process` | kill the running process |
