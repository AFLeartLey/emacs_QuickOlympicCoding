...
;;   C-c M-q p                             ; toggle the test side panel
;;   C-c M-q r                             ; compile & run all tests
;;   C-c M-q n                             ; add a new test (then edit input)
...
mutating command (e.g. `C-c M-q n' without a prior run)."
...
(defun quickolympic--ensure-idle (session)
  ...
  (let ((proc (quickolympic-session-process session)))
    (when (and (processp proc) (process-live-p proc))
      (user-error "A test is already running; kill it first (C-c M-q k)"))))
...
(defvar quickolympic-prefix-map
  (let ((map (make-sparse-keymap)))
    ...
    map)
  "QuickOlympic prefix keymap (C-c M-q).")

(defvar quickolympic-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c M-q") quickolympic-prefix-map)
    map)
