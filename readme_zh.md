...
| 进程终止 | ✅ 已实现 | `C-c q k`，Windows/Linux 平台分派 |
| 交互题（comint） | ⏳ 规划 | 尚未实现 |
| 对拍 Stress | ⏳ 规划 | 尚未实现 |
...
1. **新增测试**：`C-c q n` → 弹出编辑缓冲区，粘贴样例输入，`C-c C-c` 保存。
   重复添加多个测试。测试会存入 `A.cpp.tests`。
2. **编译并运行**：`C-c q r` → 右侧出现测试面板，逐个编译运行全部测试。
...
### 源码缓冲区（`quickolympic-mode`，前缀 `C-c q`）

| 按键 | 命令 | 功能 |
|---|---|---|
| `C-c q r` | `quickolympic-run` | 编译并运行全部测试 |
| `C-c q R` | `quickolympic-run-current-test` | 只运行当前测试 |
| `C-c q p` | `quickolympic-toggle-panel` | 显示 / 隐藏测试侧边栏 |
| `C-c q n` | `quickolympic-new-test` | 新增测试（随后编辑输入） |
| `C-c q e` | `quickolympic-edit-test` | 编辑当前测试 |
| `C-c q k` | `quickolympic-kill-process` | 终止运行中的进程 |
